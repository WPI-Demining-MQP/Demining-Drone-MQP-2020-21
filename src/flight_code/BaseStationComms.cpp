...
uint16_t parse_msg_mine(packet_t * packet, mine_t* mine_ptr) {
    mine_ptr->lat = *((int32_t*)((packet->data) + 2));   // grab bytes 2,3,4,5, cast them to a single int32_t
    mine_ptr->lon = *((int32_t*)((packet->data) + 6));   // grab bytes 6,7,8,9, cast them to a single int32_t
    mine_ptr->escape_lat = *((int32_t*)((packet->data) + 10));  // grab bytes 10,11,12,13, cast them to a single int32_t
    mine_ptr->escape_lon = *((int32_t*)((packet->data) + 14));  // grab bytes 14,15,16,17, cast them to a single int32_t
